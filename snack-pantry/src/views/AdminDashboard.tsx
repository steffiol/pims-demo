...
export default function AdminDashboard() {
	...
	const pct = Math.min(100, Math.round((state.budget.spent / state.budget.limit) * 100))
	return (
		<Container maxWidth="lg" sx={{ py: 4 }}>
			<Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 3 }}>
				...
				<Box sx={{ gridColumn: '1 / -1' }}>
					<Card>
						<CardContent>
							<Typography variant="h6" sx={{ fontWeight: 700, mb: 1 }}>Budget Pressure Gauge</Typography>
							<LinearProgress variant="determinate" value={pct} sx={{ height: 12, borderRadius: 6 }} color={pct > 85 ? 'error' : pct > 60 ? 'warning' : 'success'} />
						</CardContent>
					</Card>
				</Box>
			</Box>
